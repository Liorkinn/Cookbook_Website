...
1. Для того, чтобы ваш сайт запускался на Docker - для начала необходимо зарегестрироваться на DockerHub, создать в нём репозиторий. Далее, установить Docker Desktop и WSL. (Без WSL ваш Docker не запустится.) [В файле .env находятся системные переменные, такие как порты, название «image» находящихся на dockerhub и т.д., которые мы инжектируем в наш docker-compose файл.]
